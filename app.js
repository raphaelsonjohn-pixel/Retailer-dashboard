// BomaWave v3.1 — Multi-Store + POS Offline (FULL VERSION)
  ...
  if (FORCE_DEV_MODE) {
    ...
    if (payload.action === 'verify_otp') {
      ...
      if (existingUser) {
        return {
          ...
          user_exists: true,
          user: existingUser,
          message: 'OTP imethibitishwa (DEV MODE)'
        };
      }
      ...
      return {
        ...
        user_exists: false,
        user: null,
        message: 'OTP imethibitishwa. Kamilisha usajili.'
      };
    }
    ...
    if (payload.action === 'complete_registration') {
      const { data: newUser, error } = await sb
        ...
        .insert([{
          ...
          min_delivery_amount: payload.min_delivery_amount || 0,
      ...
      if (error) {
        ...
        return {
          success: false,
          message: 'Tatizo la kuunda akaunti: ' + error.message
        };
      }
    ...
    if (payload.action === 'reset_pin') {
      ...
      if (error) {
        return {
          success: false,
          message: 'Tatizo la kubadilisha PIN'
        };
      }
...
window.App = {
  ...
  logout() {
    ...
    S = {
      ...
      supervisorOf: null, _savedStoreId: null, resendTimer: null,
  ...
  renderApp() {
    ...
    const lswSw = $('lsw-sw');
    const lswEn = $('lsw-en');
    if(lswSw) lswSw.classList.toggle('on', S.lang === 'sw');
    if(lswEn) lswEn.classList.toggle('on', S.lang === 'en');
  ...
  toggleSidebar() {
    ...
    if(sovl) sovl.classList.toggle('active');
  },
  closeSidebar() {
    const sidebar = $('sidebar');
    const sovl = $('sovl');
    if(sidebar) sidebar.classList.remove('open');
    if(sovl) sovl.classList.remove('active');
  },

  toggleNotif() {
    const ndd = $('ndd');
    if(ndd) ndd.classList.toggle('open');
  },
        ...
        <div class="card">
          <div class="cp">
            ...
            <div class="tw"><table class="dt">
              <thead><tr><th>REF</th><th>${S.lang === 'sw' ? 'HALI' : 'STATUS'}</th><th>${S.lang === 'sw' ? 'JUMLA' : 'TOTAL'}</th><th>${S.lang === 'sw' ? 'TAREHE' : 'DATE'}</th></thead>
  ...
  toggleCart() {
    const cpanel = $('cpanel');
    if(cpanel) cpanel.classList.toggle('open');
  },
  ...
  // ══════════════════════════════════════════════════════════
  //  POS (POINT OF SALE) - FULL FEATURES
  // ══════════════════════════════════════════════════════════
  async pagePOS() {
    const uid = S.user.id;
  ...
  posCalc() {
    ...
    if (sell > 0 && qty > 0) {
      ...
      if(calcMargin) calcMargin.style.color = margin >= 20 ? 'var(--g700)' : margin >= 10 ? 'var(--amber)' : 'var(--red)';
    } else {
  ...
  async recordSale() {
    ...
    const sBuy = $('s-buy');
    const sSell = $('s-sell');
    if(sProd) sProd.value = '';
    if(sQty) sQty.value = '1';
    if(sBuy) sBuy.value = '';
    if(sSell) sSell.value = '';
    const posCalc = $('pos-calc');
    if(posCalc) posCalc.style.display = 'none';
    App.pagePOS();
  },
...
// ── Inject styles ─────────────────────────────────────────────
const _extraCSS = document.createElement('style');
_extraCSS.textContent = `
/* CSS yako hapa - sawa na ilivyo kwenye code yako original */
`;
